...
df_q1 <- 
  df_michelson %>%
    ...
    ) %>% #I've tried count multiple times, but it gives an error.
    glimpse()
...
# act_error <-
#   df_q2 %>% 
#   summarise(mean = round(mean(VelocityVacuum), -1)) %>% 
#   mutate(abs(. - LIGHTSPEED_VACUUM)) %>% 
# 
# 
# offset <- act_error - LIGHTSPEED_PM %>% 
#   print()
...
**Observations**:
- He was further away from the true value than he expected to be.
